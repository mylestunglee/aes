...
The tool uses Boolean tensors as data structures to store schedules and argumentation constructs. We will use the operators below in computing conflict-freeness, stability and aggregating explanations. The definitions below share notions with linear algebra over matrices.
\begin{definition}	
...
\begin{algorithm}[H]
	\caption{}
	\begin{algorithmic}[1]
		\Function{Explain-Efficiency}{$\mathbf{p}$, $\mathbf{C}$, $\mathbf{u}$, $\mathbf{c}$}
