...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\draw[arrow](es) -- (end);
	\end{tikzpicture}}
	\caption{The flowchart summaries the required execution order of sub-functions in the \textsc{Full-Precomputation-Explain} algorithm.}
\end{figure}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Function{Explain-Stability}{$\mathbf{x}$, $\rightsquigarrow$, $\bar{\mathbf{u}}$, $\bar{\mathbf{c}}$}
			...
			\State $\mathbf{c}$ $\gets$ \textsc{Compute-Conflicts}($\mathbf{x}$, $\rightsquigarrow$, $\bar{\mathbf{c}}$)
		...
		\EndFunction
		\Function{Compute-Conflicts}{$\mathbf{x}$, $\rightsquigarrow$, $\bar{\mathbf{c}}$}
