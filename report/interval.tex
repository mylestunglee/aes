...
\begin{definition}
	A schedule property $P$ is stability-modelable iff there exists a framework that stability-models $P$.
\end{definition}
...
\begin{theorem}[\textbf{Interval scheduling is stability-modelable}]
	Let $\Lambda(S)$ iff $\forall\lambda\in\{\alpha,\beta,\gamma,\delta,\varepsilon,\zeta\}\ \lambda(S)$. $\Lambda$ is stability-modelable.
	...
	\begin{proof}\ 
		\begin{enumerate}
			...
			\item $\bigcup_{\lambda\in\{\alpha,\beta,\gamma,\delta,\varepsilon,\zeta\}}\rightsquigarrow_\lambda$ stability-models $\Lambda$\\\indent\hfill lemma \ref{stabilityalpha}, lemma \ref{conflictfreenessbeta}, 1, 2, 3, lemma \ref{intervalstabilitydependence}, theorem \ref{modelling}
			\item $\Lambda$ is stability-modelable \hfill 5
		\end{enumerate}
	\end{proof}
\end{theorem}